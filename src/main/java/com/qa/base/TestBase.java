...
public class TestBase {
	...
	public void suiteSetup() throws Exception {
		 ...
		 * new ChromeDriver(options); } else if (BROWSER.equals("IE")) {
		 * WebDriverManager.iedriver().setup(); driver = new InternetExplorerDriver(); }
		 * else { throw new RuntimeException("Browser type unsupported"); }
		 * 
		 * // Setting implicit wait driver.manage().timeouts().implicitlyWait(10,
		 * TimeUnit.SECONDS); driver.manage().window().maximize(); // Setting
		 * WebDriverWait with max timeout value of 20 seconds wait = new
